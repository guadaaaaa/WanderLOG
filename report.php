...
?>
    ...
    <div style="justify-content: center;">
        ...
        $entries = $mysqli->query("SELECT * from tblentry as entry INNER JOIN tbluseraccount as ua ON entry.acctid = ua.acctid") or die($mysqli->error);
        foreach($entries as $entry){
            echo '<div style="margin: 10px; text-align: center; display: inline-block; line-height: 10px; width: 250px; height: 150px; border: 1px solid black; margin-top: 1.5%; padding: 10px; border-radius: 10px; background-color: white">
    ...
    <div style="justify-content: center;">
        ...
        <?php
        $mysqli = new mysqli('localhost', 'root','','dbobandof1');
        $entries = $mysqli->query("SELECT * from tblreviewlocation as review INNER JOIN tbluseraccount as ua ON review.acctid = ua.acctid") or die($mysqli->error);
        foreach($entries as $entry){
            echo '<div style="margin: 10px; text-align: center; display: inline-block; line-height: 10px; width: 250px; height: 150px; border: 1px solid black; margin-top: 1.5%; padding: 10px; border-radius: 10px; background-color: white">
                <h3 style="color: #b67352"><b>'.$entry['username'].'</b></h3>
                <p style="font-size: 15px">'.$entry['review_content'].'</p>
                <hr style="width: 75%; border-color: black; 0; margin: auto;">
